...
- [Алгоритм](#algo)
  - [Предварительная обработка и выбор контура](#algo-preprocessing)
  - [Модифицированный фильтр Kuwahara](#algo-kuwahara)
...
## <a name="algo-preprocessing"></a> Предварительная обработка и выбор контура

Само мобой первый этап был преобразование изображения в бинарное, но даже не буду останавливаться на этом.

Далее были вырезаны цифры из изображений по их положению на изображении. Благо мы точно знаем их положение и они не меняются. В реальной жизни такое редкость, но задача выделение отдельных обьектов достаточно тривиальна. Взять тот же [Breadth-first search](https://en.wikipedia.org/wiki/Breadth-first_search) и снова таки не является предметом эксперимента.
...
Кроме того, изображение сильно зашумлено гауссовым шумом. Для того чтоб получить более вменяемое изображение был использован специально модифицированный фильтр Kuwahara c ядром 2 [[2]](#literature-2). В результате получим следующие изображения:
...
Есть еще некоторые проблемы в результирующем решении. Контуры вышли весьма угловаты. Это повлечет за собой повышенное количество шаблонов в базе для распознавание. Сгладим немного контуры гауссовым размытием [[3]](#literature-3). Подобранные значения *сигма = 3* и *ядро = 4*. Выделим все присутствующие контуры на изображении.
...
## <a name="algo-kuwahara"></a> Модифицированный фильтр Kuwahara
...
Как показала практика, некоторые цифры более похижи между собой, а некоторые меньше. Тут проявляется первый недостаток выбранного алгоритма обучения по прецендентам (алгоритм ближайшего соседа). В отличии от нейронных сетей ему не хватает адаптивности. Добавим ее вручную. Введем веса для каждого класса.
...
Тут виден серьезный недостаток метода. Если бы классов было значительно больше или вообще неопределнное количество(кластеризация) -- не о какой адаптивности речи идти и не могло бы.
